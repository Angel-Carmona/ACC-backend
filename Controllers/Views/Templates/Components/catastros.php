...
tabla('id_catastro_relations', 'catastro_relations');
...
<div class="p-3 col-lg-12 card shadow" style="min-height:80vh;max-height:auto" id="myTabContent">
	<div class="tab-pan fade col-lg-12 show borde pl-3 pr-3 pt-2 pb-2" id="home" role="tabpanel"
		...
		<div class="px-3 table-responsive">
			<table id="catastros" class="display compact bg-white">
		...
		<script>
			...
			let catastros = window.callapi(JSON.stringify([{
				name: "action",
				value: "getCatastros"
			}]));
			...
			for (i = 0; i < 1; i++) {
				...
				for (const key in catastros[i]) {

					if (Object.hasOwnProperty.call(catastros[i], key)) {
						...
						if (["id_provincia"].includes(key)) {
							miniCol.render = function(data) {
								var provincia = provincias.filter(e => parseInt(e.id_provincia) === parseInt(data))[0]
									.provincia;
								return provincia;
							};
						}
			...
			$(document).ready(function() {
				...
				function addTabla(catastros) {

					try {
						catastros = JSON.parse(catastros);
					} catch (error) {
					...
					for (i = 0; i < 1; i++) {
						for (const key in catastros[i]) {
							if (Object.hasOwnProperty.call(catastros[i], key)) {
								const element = catastros[i][key];
								thead +=
									'<td controls="example" style="width:0px;text-align:left;text-transform:uppercase;font-weight:bold;">' +
									key.replace(/id_/g, '') + '</td>';
							}
						}
					}
					thead += '</tr>';
					jQuery('#catastros thead').empty().append(thead);

					table = jQuery('#catastros').DataTable({
							select: true,
							paging: true,
							lengthMenu: [
								[5, 8, 10, 25, 50, -1],
							...
							buttons: ['select', {
								...
								action: function(e, dt, node, config) {
									window.location.href =
										'/excel/plugins/importar-excel/importBancos/index.php';
								}
							} <?php endif; ?> , {
								extend: 'colvis',
								columns: ':not(.noVis)'
						...
						.on('dblclick', 'tbody td', function() {
							...
							swal({
								title: "",
								text: "No tienes permisos para editar, consulta con tu administrador.",
								icon: 'error',
								buttons: false,
								timer: 2000
							});
						...
						.on('mouseup', 'tbody tr', function(e) {
							...
							if (e.which !== 3) {
								if ($(this).hasClass('active')) {
									...
									seleccionados = seleccionados.filter((value) => value[config.primaryKey] !== row[
										config.primaryKey]);
								} else {
				...
				<?php if(!CLIENT) : ?>
				let selProvincias =
					'<select name="id_provincia" id="edi-id_provincia" class="custom-select form-select">';
				...
				for (let index = 0; index < conceptos.length; index++) {
					...
					selConceptos += '<option value="' + element.id_concepto + '">' + element.concepto_code + '. ' +
						element
						.concepto + '</option>';
				}
				selConceptos += '</select>';
					...
					$.contextMenu({
						...
						callback: function(key, options) {
							...
							if (key == 'edit') {
								...
								jQuery('.guardar-registro').unbind('click').off('click').on('click',
									function() {
								...
								if (confirm('desea eliminar el registro ' + row[config
										.primaryKey]) == true) {
									jQuery('#edi-' + config.primaryKey).val(row[config.primaryKey]);
									jQuery('#editarPala').append(
										'<input hidden name="eliminar" value="1">');
									jQuery('#editarPala').submit();
								}
							}
						},
				...
				jQuery('#editarPala').on('submit', function(e) {
					...
					catastros = window.callapi(JSON.stringify([{
						name: "action",
						value: "getCatastros"
					}]), `addTabla`);
