...
tabla('id_banco_relations', 'bancos_relations');
...
<div class="p-3 col-lg-12 card shadow" style="min-height:80vh;max-height:auto" id="myTabContent">
	<div class="tab-pan fade col-lg-12 show borde pl-3 pr-3 pt-2 pb-2" id="home" role="tabpanel"
		...
		<div class="px-3 table-responsive">
			<table id="bancos" class="display compact bg-white">
...
<script>
	...
	let bancos = window.callapi(JSON.stringify([{
		name: "action",
		value: "getBancos"
	}]));
	...
	for (i = 0; i < 1; i++) {
		...
		for (const key in bancos[i]) {

			if (Object.hasOwnProperty.call(bancos[i], key)) {
	...
	$(document).ready(function() {
		...
		function addTabla(bancos) {
			...
			thead += '</tr>';
			jQuery('#bancos thead').empty().append(thead);

			table = jQuery('#bancos').DataTable({
				select: true,
				paging: true,
				lengthMenu: [
					[5, 8, 10, 25, 50, -1],
			...
			$.contextMenu({
				...
				callback: function(key, options) {
					...
					if (key == 'edit') {
						...
						if (confirm('desea eliminar el registro ' + row[config.primaryKey]) ==
							true) {
		...
		jQuery('#editarPala').on('submit', function(e) {
			...
			bancos = window.callapi(JSON.stringify([{
				name: "action",
				value: "getBancos"
			}]), `addTabla`);
